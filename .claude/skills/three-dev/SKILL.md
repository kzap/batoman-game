...
| Offline asset tooling | `tools/` | co-located `*.test.ts` |
...
- Sim state is plain data. No class instances holding Three objects, no DOM handles, no `Date.now()`.
- `World.step()` advances exactly one tick. It takes an `InputFrame`, never reads devices.
- Everything the renderer needs is in `World.snapshot()`. The renderer never calls into `World`.
- Randomness comes from a seeded PRNG stored in world state. `Math.random()` is banned in `core`/`game`.
...
- `tools/recut` segments AI-generated sheets into frames using flood-fill from the border, connected
  components, and row clustering; a committed `*.recipe.json` records the human-reviewed result.
- `tools/pack` writes atlases to `public/assets/atlases/` (gitignored; regenerated by `npm run assets`).
- `tools/validate` and `tools/validate/budget.ts` fail the build on malformed content or oversized payload.
  Budgets are in `tools/config.ts`; raise them in a PR with a stated reason.
...
npm run assets         # regenerate atlases from art-source/ + recipes (Phase 1+)
```
...
- `postprocessing` (pmndrs) is the effects library, not `three/addons/postprocessing`.
